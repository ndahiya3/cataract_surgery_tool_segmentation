...
int main(int argc, char *argv[])
{
  ...
  for (int i=1; i<26; i++) {
      ...
      while (getline(data,line)) {
          ...
          // Store frame number for each tool that is present
          bool no_tool_present=true;
          for (int j=1; j<vec.size(); j++) { // 1st token is frame number
              ...
              if (isToolPresent==1) {
                  frame_num_for_tools[j-1].push_back(frame_num);
                  no_tool_present=false;
                }
            }
          if (no_tool_present)
            frame_num_for_tools[21].push_back(frame_num);
        }
      data.close();
      // We have frame number for each tool in current file now. Write them
      for (int k=0; k<22; k++) {
          file[k] << to_string(i);
